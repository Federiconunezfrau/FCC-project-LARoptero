...
\begin{figure}[!t]
    ...
    \caption{En cada tick se llama al dispatcher el cual selecciona la tarea a ejectuar. Todas las tareas se almacenan en la lista del atributo mTaskList\_ del scheduler. En la imagen se muestra la evolución de la lista a medida que ocurren los ticks. En cada tick, el atributo mDelayTicks\_ de todas las tareas se decrementa en 1. En caso de que se detecte alguna tarea que alcanza un mDelayTicks\_ = 0, esta es ejecutada. Para finalizar, se vuelve a setear mDelayTicks\_ = mPeriodTicks\_, permitiendo que las tareas puedan volver a ser ejecutadas.}
...
\begin{subequations}
    \begin{align}
        \theta[n] &= \alpha \ atan_2(-a_x[n], a_z[n]) + (1 - \alpha) \left( \Delta T \ g_y[n] + \theta[n-1] \right)\\
